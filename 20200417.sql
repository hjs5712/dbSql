 ...
 a exclusive or b a=true, b=ture ==> false
 ...
 WHERE sal >= 1000
   ...
   WHERE hiredate >= TO_DATE('19820101') 
     AND hiredate <=  TO_DATE('19820101');

IN 연산자
컬럼 특정값 IN (값1, 값2,..)
컬럼이나 특정값이 괄호안데 값중에 하나라도 일치를 하면 TRUE
...
ORDER BY 컬럼명 [정렬형태], 컬럼명2 [정렬형태]... --여러개 써도 됨
...
job을 기준으로 오름 차순으로 정렬하고 job이 같을경우 입사일자로 내림차순정렬
모든 데이터 조회
